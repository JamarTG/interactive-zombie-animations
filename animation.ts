...
type Action  = "walk" | "die" | "attack" | "idle";
...
idleButton.addEventListener("click", (event) =>
  changeAction((event.target as HTMLButtonElement).name as Action)
);
...
const draw = () => {
  ...
  zombieImg.onload = () => {
    ...
    ctx.drawImage(
      zombieImg,
      canvas.width / 2 - (isGenderMale ? 0 : 50),
      canvas.height / 2 - (isGenderMale ? 0 : 20),
      spriteWidth,
      spriteHeight
    );
